    ...
    if (user.role != "reviewer") {
      ...
      if (papers.length == 0) {
        ...
        papers.forEach(async (paper) => {
          let review = await fetch("/api/review?paperId=" + paper.id)
          ...
          card.innerHTML = `
              <h2>${paper.title}</h2>
              <h3>Status: <i style="color:${paper.status == "accepted" ? "green" : "red"}">${paper.status}</i></h3>
            `;
          card.addEventListener("click", () => {
            ...
            <h2>${paper.title}</h2>
            <input type="radio" id="accepted" name="status" value="accepted" ${paper.status == "accepted" ? "checked" : null}>
            <label for="accepted">Accepted</label><br>
            <input type="radio" id="rejected" name="status" value="rejected" ${paper.status == "rejected" ? "checked" : null}>
...
async function save(id) {
  ...
  let review = await fetch(`../api/review?paperId=${id}`);
  let newReview = {
    id: review.id,
